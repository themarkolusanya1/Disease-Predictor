...
if st.session_state.show_results:
    ...
    if not desc.empty:
        ...
        st.info(desc.iloc[0]["Description"])

    if not prec.empty:
        st.subheader("💊 Recommended Precautions")

        for i in range(1, 5):
            precaution = prec.iloc[0][f"Precaution_{i}"]

            if pd.notna(precaution):
                st.write(f"✅ {precaution}")

    st.warning(
        "⚠ **Disclaimer:** This prediction is generated by a Machine Learning model for educational purposes only. It is not a medical diagnosis. Please consult a qualified healthcare professional."
    )
